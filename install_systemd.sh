...
"${HOME}/.local/bin/trademl" node --workspace-root "${WORKSPACE_ROOT}" --config "${WORKSPACE_ROOT}/node.yml" --env-file "${WORKSPACE_ROOT}/.env" install-service --service-path "${SERVICE_PATH}"
